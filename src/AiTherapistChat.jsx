...
function AiTherapistChat() {
  ...
  const sendMessage = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://localhost:8000/ai-therapist', {
        method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 600, margin: '40px auto', padding: 20, background: '#fff', borderRadius: 12, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
      ...
      <div style={{ minHeight: 200, marginBottom: 16 }}>
        ...
        {loading && <div style={{ color: '#888' }}>Dr. Sophia is typing...</div>}
      </div>
